...
const Category = ({ title, color, data, refreshData, categories }: CategoryProps) => {
  ...
  const handleUpdateTask = async (taskData: Partial<ITask> & { id: string }) => {
    try {
  ...
  return (
    <>
      <div className="flex flex-col gap-4 p-5 bg-primary-foreground/8 rounded-2xl max-w-60 w-max flex-1 min-w-50">
      <header className={`flex justify-between`} style={{color}}>
        <div className="title flex items-center gap-2">
            <div className={`dot w-2 aspect-square rounded-full`} style={{background: color}}></div>
            <h3 className="font-semibold text-sm">{title}</h3>
        </div>
        <div className="total font=medium text-sm">
            {data.length}
        </div>
      </header>
      <div className="tasks flex flex-col gap-2">
        {
          data.map((task, index) => (
            <div className="task flex items-center gap-3 p-3 bg-primary-foreground rounded-xl" key={index} onClick={() => handleTaskClick(task)}>
                <div className="dot w-2 aspect-square rounded-full" style={{background: color}}></div>
                <div className="info flex-1">
                  <p className="font-medium text-sm text-muted">{task.title}</p>
