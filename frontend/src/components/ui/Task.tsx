...
const Task =
  ({ task, index, color, handleUpdateTaskStatus, handleTaskClick, getPriorityColorClass }: TaskProps) => {
    return (
        ...
        {(provided, snapshot) => (
          <div
            ...
            className={`flex flex-col gap-2 p-3 rounded-xl cursor-pointer ${
              task.isCompleted ? "bg-success hover:bg-success/80" : "bg-primary-foreground hover:bg-primary-foreground/80"
          ...
          >
            <header className="flex items-center gap-2 justify-between">
              <div className="w-2 aspect-square rounded-full" style={{ background: color }}></div>
              <div className="actions flex items-center gap-1">
                <button
                  className={`w-6 aspect-square rounded-full flex justify-center items-center cursor-pointer ${
                    task.isCompleted ? "bg-primary-foreground text-success" : "bg-success"
                  }`}
                  onClick={(e) => {
                    e.stopPropagation();
                ...
                <button
                  className={`w-7 aspect-square flex justify-center items-center rounded-lg cursor-pointer ${
                    task.isCompleted ? "bg-primary-foreground/8" : "bg-background/5"
                  }`}
                  onClick={(e) => {
                    e.stopPropagation();
                ...
                >
                  <MoreVertical
                    ...
                    className={`rounded-sm ${
                      task.isCompleted ? "text-primary-foreground" : "text-muted-foreground"
                    }`}
                  />
                </button>
              </div>
            ...
            >
              ...
              <span
                className={`flex items-center gap-1 ${getPriorityColorClass(task.priority)}`}
              >
                <Flag size={14} />
                {task.priority === "low"
                  ? "Baixa"
